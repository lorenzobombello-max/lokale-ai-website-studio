...
function App() {
  const architecture = [
    { icon: LayoutTemplate, title: 'Template engine', text: 'Tien beroepen met meerdere layouts, secties en conversiedoelen.' },
    { icon: Wand2, title: 'AI editor', text: 'Modulaire editor met live preview, contentblokken en gratis preview mode.' },
    { icon: CreditCard, title: 'Billing layer', text: 'Stripe Checkout voorbereidingen voor abonnementen, bankkaart, Bancontact en QR.' },
    { icon: Globe2, title: 'Domain automation', text: 'Domeinchecker en DNS-record voorbereiding voor automatische koppeling.' },
    { icon: Database, title: 'Project storage', text: 'Structuur voor opgeslagen projecten, statussen, revisies en dashboardbeheer.' },
    { icon: Lock, title: 'Account systeem', text: 'Architectuur voor magic links, OAuth, rollen en veilige sessies.' },
  ];
    ...
    <main className="min-h-screen overflow-hidden bg-cream text-ink">
      ...
      <header className="mx-auto flex max-w-7xl items-center justify-between px-4 py-6 sm:px-6 lg:px-8">
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="text-lg font-semibold tracking-tight">Lokale AI Website Studio</p>
            <p className="text-xs font-medium uppercase tracking-[0.25em] text-slate-500">SaaS builder</p>
          </div>
        </div>
        <nav className="hidden items-center gap-8 text-sm font-medium text-slate-600 md:flex">
          <a href="#templates">Templates</a>
          <a href="#editor">Editor</a>
          <a href="#billing">Prijzen</a>
          <a href="#dashboard">Dashboard</a>
        </nav>
        <Button variant="secondary">Login / Start gratis</Button>
      </header>
        ...
        <div className="grid items-center gap-12 lg:grid-cols-[1.05fr_0.95fr]">
          <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7 }}>
            ...
            <div className="mt-9 flex flex-col gap-3 sm:flex-row">
              <Button className="px-7 py-4 text-base">Start met AI template <ArrowRight className="ml-2 h-5 w-5" /></Button>
              <Button variant="secondary" className="px-7 py-4 text-base">Bekijk dashboard</Button>
            </div>
      ...
      <footer className="mx-auto max-w-7xl px-4 pb-10 sm:px-6 lg:px-8">
        <div className="rounded-[2rem] bg-ink p-8 text-white md:flex md:items-center md:justify-between">
          <div>
            <p className="text-2xl font-semibold">Lokale AI Website Studio</p>
            <p className="mt-2 text-white/60">Premium AI websites voor lokale zelfstandigen.</p>
          </div>
          <div className="mt-6 flex flex-wrap gap-3 md:mt-0">
            <span className="rounded-full bg-white/10 px-4 py-2 text-sm">React</span>
            <span className="rounded-full bg-white/10 px-4 py-2 text-sm">Tailwind</span>
            <span className="rounded-full bg-white/10 px-4 py-2 text-sm">Stripe-ready</span>
            <span className="rounded-full bg-white/10 px-4 py-2 text-sm">Domain-ready</span>
          </div>
        </div>
      </footer>
    </main>
