      ...
      <aside className="rounded-[2rem] border border-white/80 bg-white/80 p-5 shadow-soft backdrop-blur">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-slate-500">Gratis bouwen, betalen bij publicatie.</p>
          </div>
        </div>
        <div className="mt-6 space-y-4">
          <label className="text-sm font-semibold text-slate-600" htmlFor="profession">Beroep</label>
          <select
            id="profession"
            value={selectedId}
          ...
          >
            {professionTemplates.map((template) => <option key={template.id} value={template.id}>{template.title}</option>)}
          </select>
          <div className="rounded-3xl bg-slate-50 p-4">
            <p className="mb-3 flex items-center gap-2 text-sm font-semibold text-ink"><Palette className="h-4 w-4" /> Layouts</p>
            <div className="space-y-2">
              {selected.layouts.map((layout) => (
                <button key={layout.id} className="w-full rounded-2xl border border-slate-100 bg-white p-3 text-left transition hover:border-indigo-200 hover:shadow-soft">
                  <p className="text-sm font-semibold text-ink">{layout.name}</p>
                  <p className="mt-1 text-xs text-slate-500">{layout.features.join(', ')}</p>
                </button>
              ))}
            </div>
          </div>
          <div className="rounded-3xl border border-amber-200 bg-amber-50 p-4 text-sm leading-6 text-amber-900">
            <strong>Preview mode:</strong> onbeperkt testen met share-link. Publiceren, domein activeren en automatische verlenging starten pas na Stripe betaling.
          </div>
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-1">
            <Button variant="secondary"><Eye className="mr-2 h-4 w-4" /> Preview gratis</Button>
            <Button><Lock className="mr-2 h-4 w-4" /> Publiceer na betaling</Button>
          </div>
        </div>
      </aside>
        ...
        <div className={`bg-gradient-to-br ${selected.gradient} p-6 md:p-10`}>
          <div className="rounded-[2rem] bg-white/85 p-6 shadow-soft backdrop-blur md:p-10">
            <nav className="flex flex-wrap items-center justify-between gap-4">
              <div className="flex items-center gap-3"><span className="text-4xl">{selected.previewImage}</span><div><p className="font-semibold text-ink">{selected.title}</p><p className="text-xs text-slate-500">AI gegenereerde website</p></div></div>
              <Button className="px-4 py-2">{selected.conversionGoal}</Button>
            </nav>
            <div className="mt-14 grid items-center gap-10 lg:grid-cols-[1fr_0.8fr]">
              <div>
                <p className="mb-4 inline-flex rounded-full bg-white px-4 py-2 text-sm font-semibold text-indigo-500 shadow-soft">Premium lokale website</p>
                <h3 className="text-4xl font-semibold tracking-tight text-ink md:text-6xl">{selected.tagline}</h3>
                <p className="mt-6 max-w-2xl text-lg leading-8 text-slate-600">{selected.description}</p>
                <div className="mt-8 flex flex-wrap gap-3">{selected.sections.slice(0, 3).map((section) => <span key={section} className="rounded-full bg-slate-100 px-4 py-2 text-sm text-slate-600">{section}</span>)}</div>
              </div>
              <div className="rounded-[2rem] bg-ink p-5 text-white shadow-soft">
                <p className="text-sm text-white/60">Website score</p>
                <p className="mt-2 text-5xl font-semibold">94%</p>
                <div className="mt-6 space-y-3">
                  {selected.sections.map((section) => <div key={section} className="rounded-2xl bg-white/10 p-3 text-sm">✓ {section}</div>)}
                </div>
              </div>
            </div>
          </div>
